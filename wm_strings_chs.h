/**
 * wm_strings_chs.h
 * Simplified Chinese strings for
...
#ifndef _WM_STRINGS_CHS_H_
#define _WM_STRINGS_CHS_H_
...
"<form action='/update'  method='get'><button>更新固件</button></form><br/>\n",// MENU_UPDATE
"<hr><br/>" // MENU_SEP
};
...
const char HTTP_FORM_WIFI[]        PROGMEM = "<label for='s'>SSID</label><input id='s' name='s' maxlength='32' autocorrect='off' autocapitalize='none' placeholder='{v}'><br/><label for='p'>密码</label><input id='p' name='p' maxlength='64' type='password' placeholder='{p}'><input type='checkbox' id='showpass' onclick='f()'> <label for='showpass'>显示密码</label><br/>";
...
const char HTTP_SAVED[]            PROGMEM = "<div class='msg'>正在保存凭据<br/>正在连接到网络<br />如果连接失败，请重新进入WiFi管理中心再试</div>";
const char HTTP_PARAMSAVED[]       PROGMEM = "<div class='msg S'>已保存<br/></div>";
const char HTTP_END[]              PROGMEM = "</div></body></html>";
const char HTTP_ERASEBTN[]         PROGMEM = "<br/><form action='/erase' method='get'><button class='D'>擦除 WiFi 设置</button></form>";
...
const char HTTP_STATUS_NONE[]      PROGMEM = "<div class='msg'>尚未配置接入点</div>";
...
// @todo remove html elements from progmem, repetetive strings
	...
	const char HTTP_INFO_chiprev[]    PROGMEM = "<dt>芯片 Rev</dt><dd>{1}</dd>";
  	const char HTTP_INFO_lastreset[]  PROGMEM = "<dt>最后一次复位原因</dt><dd>CPU0: {1}<br/>CPU1: {2}</dd>";
  	const char HTTP_INFO_aphost[]     PROGMEM = "<dt>接入点主机名</dt><dd>{1}</dd>";
    const char HTTP_INFO_psrsize[]    PROGMEM = "<dt>PSRAM 大小</dt><dd>{1} bytes</dd>";
	...
	const char HTTP_INFO_lastreset[]  PROGMEM = "<dt>最后一次复位原因</dt><dd>{1}</dd>";
	const char HTTP_INFO_flashsize[]  PROGMEM = "<dt>真实 Flash 大小</dt><dd>{1} bytes</dd>";
#endif
...
const char HTTP_INFO_aboutver[]     PROGMEM = "<dt>WiFi管理中心</dt><dd>{1}</dd>";
...
const char S_brand[]              PROGMEM = "WiFi管理中心";
...
const char S_nonetworks[]         PROGMEM = "找不到可用的接入点，请刷新已再次扫描";
...
const char S_resetting[]          PROGMEM = "模块将会在几秒钟内重置";
const char S_closing[]            PROGMEM = "你可以关闭此页，WiFi管理中心将继续运行";
